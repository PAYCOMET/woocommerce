...
function hwn_add_thankyou_custom_text($order_id) {
    ...
    if($order_id->get_payment_method() == "paycomet_multibanco"){
        if ($order_id->get_meta("PayTPV_methodData")) {
            $datos = $order_id->get_meta("PayTPV_methodData");
...
function hwn_add_thankyou_mbway_confirm($order_id) {
    if ($order_id->get_payment_method() == "paycomet_mbway"){
        ?>
